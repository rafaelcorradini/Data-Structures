...
node *searchElem(abb *T, char k[30], node *n) {
...
boolean expandExternal(abb *T, node *n) {
	...
	n->rightChild = NULL;
	if (isExternal(n)) {
		n->leftChild = malloc(sizeof(node));
		n->rightChild = malloc(sizeof(node));
...
tipo_elem removeAboveExternal(abb *T, node *n) {
	node *v;
	tipo_elem el = n->elem;
	if (n == T->root) {
...
node *replaceElem(tipo_elem el, node *n) {
	if (isExternal(n) || n == NULL) return NULL;
	n->elem = el;
	return n;
}

tipo_elem removeElem(abb *T, char k[30]) {
	node *s, *n;
	tipo_elem el = {"",""}, els;
...
node *insertElem(abb *T, tipo_elem el) {
	node *n;
	if (isEmpty(T)) {
